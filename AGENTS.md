...
## Local Stats fork additions

This fork is published as a separate extension/provider so it can coexist with the upstream extension:

- Extension package name: `opencode-go-copilot-stats-provider`.
- Language model vendor: `opencodegosniffer`.
- Settings prefix: `opencodegosniffer.*`.
- API key secret: `opencodegosniffer.apiKey`.
- Local stats token secret: `opencodegosniffer.localStatsToken`.

New local statistics files:

| File | Responsibility |
|------|----------------|
| `src/localStats/types.ts` | Request record and summary types. |
| `src/localStats/recorder.ts` | In-memory request recording, token/latency/chunk accounting, payload capture policy. |
| `src/localStats/server.ts` | Local/intranet HTTP server, client IP allowlist, token-authenticated dashboard and JSON API. |

New commands:

| Command | Description |
|---------|-------------|
| `opencodegosniffer.openLocalStats` | Opens the local dashboard with the generated auth token. |
| `opencodegosniffer.restartLocalStats` | Restarts the local dashboard server after changing settings. |

New settings:

| Setting | Default | Description |
|---------|---------|-------------|
| `opencodegosniffer.localStatsEnabled` | `true` | Enables the local HTTP dashboard. |
| `opencodegosniffer.localStatsPort` | `43177` | Local HTTP port. |
| `opencodegosniffer.localStatsHost` | `127.0.0.1` | Bind address. Use `0.0.0.0` for intranet exposure. |
| `opencodegosniffer.localStatsAllowedClients` | `127.0.0.1,::1` | Comma-separated client IP allowlist; supports exact IPs, IPv4 wildcards, CIDR, and dotted netmasks. |
| `opencodegosniffer.localStatsCapturePayloads` | `false` | Stores request bodies and full response text when debugging. |
| `opencodegosniffer.localStatsMaxEntries` | `200` | Maximum in-memory request records. |

Security rule: the server must bind only to `127.0.0.1` and every dashboard/API route must require the generated token.
